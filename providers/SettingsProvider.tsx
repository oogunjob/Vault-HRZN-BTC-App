...
import { getIsHandOffUseEnabled, setIsHandOffUseEnabled } from '../components/HandOffComponent';
import { useStorage } from './hooks/useStorage';
import { BitcoinUnit } from '../models/bitcoinUnits';
import { TotalWalletsBalanceKey, TotalWalletsBalancePreferredUnit } from '../components/TotalWalletsBalance';
    ...
    if (walletsInitialized) {
      try {
        if (isElectrumDisabled) {
          ...
          // Wrap in try-catch to handle any errors during connection
          try {
            BlueElectrum.connectMain();
          } catch (error: any) {
            console.error('[SettingsProvider] Error connecting to Electrum:', error?.message || error);
            // Don't throw - allow app to continue without Electrum connection
          }
        }
      } catch (error: any) {
        console.error('[SettingsProvider] Error in Electrum effect:', error?.message || error);
      }
    }
